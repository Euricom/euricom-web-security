...
app.post(
  ...
  (req, res) => {
    ...
    console.error(
      `CSP WARNING ${
        argv.reportOnly ? "(Report Only)" : ""
      }: '%s' blocked by '%s' in '%s' %s`,
      report["blocked-uri"],
      report["violated-directive"],
      report["document-uri"],
      !report["document-uri"].includes("http")
        ? `(${report["source-file"]})`
        : ""
    );
    res.send("ok");
...
app.get("*", async (req, res, next) => {
  ...
  const content = await readFile(path.join(rootPath, "index.html"), {
    encoding: "utf8",
  });
  if (!req.nonce) {
    res.send(content);
  ...
  console.log("Process index.html with nonce: %s", req.nonce);
  const contentWithNonce = content
